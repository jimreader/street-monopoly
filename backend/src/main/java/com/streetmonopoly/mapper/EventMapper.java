...
public interface EventMapper {
    ...
    Event findById(UUID id);
        ...
        @Insert("INSERT INTO event (id, name, logo_image_url, game_map_id, start_time, end_time, starting_balance, proximity_metres, max_players_per_game, unvisited_street_penalty_percent, status) " +
            "VALUES (#{id}, #{name}, #{logoImageUrl}, #{gameMapId}, #{startTime}, #{endTime}, #{startingBalance}, #{proximityMetres}, #{maxPlayersPerGame}, #{unvisitedStreetPenaltyPercent}, #{status})")
    void insert(Event event);
        ...
        @Update("UPDATE event SET name = #{name}, logo_image_url = #{logoImageUrl}, game_map_id = #{gameMapId}, start_time = #{startTime}, end_time = #{endTime}, " +
            "starting_balance = #{startingBalance}, proximity_metres = #{proximityMetres}, max_players_per_game = #{maxPlayersPerGame}, unvisited_street_penalty_percent = #{unvisitedStreetPenaltyPercent} " +
